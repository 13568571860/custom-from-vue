...
import { ElInput, ElRow, ElRadio, ElInputNumber, ElRadioGroup, ElTabs, ElTabPane, type TabPaneName, ElSelect, ElOption, ElTooltip, ElColorPicker, ElFormItem, ElForm } from 'element-plus'
...
function renderAttrItem(data: AttrMenu[], values: { [key:string]: any }) {
  ...
  data.forEach(item => {
    ...
    const form = h(ElForm, { labelWidth: 'auto' }, { default: () => children })
    item.input.forEach(r => {
      ...
      children.push(h(ElFormItem, { label: r.label || '', style: 'line-height: 0;'}, { default: () => {
        if(r.component === 'ElRadio') return renderRadio(r, values)
        else if(r.component === 'ElInputNumber') return renderInputNumber(r, values)
        else if(r.component === 'ElInput') return renderInput(r, values)
        else if(r.component === 'listInput') return renderListInput(r, values)
        else if(r.component === 'ElSelect') return renderElSelect(r, values)
        else if(r.component === 'ElColorPicker') return renderElColorPicker(r, values)
      }}))
    })
    ...
    vnodes.push(h(ElRow, { class: 'attrRow' }, { default: () => form }))
  })
  return vnodes
}
